...
function youtube_search($apikey, $search, $limit, $timeout){
...
function youtube_video($apikey, $videoID, $timeout){
	$url = "https://www.googleapis.com/youtube/v3/videos?id=".$videoID."&&part=snippet%2Cstatistics%2CcontentDetails&key=".$apikey;
	$api = get_ydata($url, $timeout);
	return $api;
}
...
if (isset($_REQUEST['search']) && isset($_REQUEST['limit'])) {
    ...
    $res_json = youtube_search($apikey, $search, $limit, $timeout);
    ...
    foreach ($res['items'] as $res) {
        switch ($res['id']['kind']) {
            case 'youtube#video':
				    ...
				    $res_json_2 = youtube_video($apikey, $videoId, $timeout);
